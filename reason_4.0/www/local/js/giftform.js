...
$(document).ready(function() {
	...
	if ($("div#giftForm.pageOne").length)
	{
		...
		$("#gift_designation_otherElement").attr('placeholder', 'Other Designation');
		if ( $("input#split_naa_detailsElement") && $("input#split_naa_detailsElement").val() ) {
			$("input#gift_designation_naaElement").val( $("input#split_naa_detailsElement").val() );
		}
		splits = $.parseJSON($("input#split_designationsElement").val());
		if ( splits && splits.other_details ) {
			$("input#gift_designation_otherElement").val( splits.other_details );
		}
	}
...
function total_split_gifts()
{
	if ($('#gift_designation_container').hasClass('split'))
	{
		...
		if ( $("#gift_designation_naa_detailsElement option:selected").val() ) {
			designations.naa_details	= $("#gift_designation_naa_detailsElement").val();
		}
		if ( $("input#gift_designation_otherElement").length !== 0 ) {
			designations.other_details 	= $("input#gift_designation_otherElement").val();
		}
