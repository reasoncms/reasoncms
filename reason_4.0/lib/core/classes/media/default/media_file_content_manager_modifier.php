<?php
/*
 * Content manager modifier for non-integrated Audio/Video/Multimedia Files
...
/**
 * Media file content manager modifier.
 * @author Matt Ryan mryan@carleton.edu
 * @author Nate White nwhite@carleton.edu
 */ 
class DefaultMediaFileContentManagerModifier implements MediaFileContentManagerModifierInterface
{
	protected $manager;
	...
	/**
	 * Displayer chrome used for the file preview.
	 */
	...
	/**
	 * Loads some javascript used by the default content manager.
	 */
	public function set_head_items()
	{		
		$this->manager->head_items->add_javascript(REASON_HTTP_BASE_PATH.'js/media_file_content_manager.js');
		if ($this->displayer_chrome)
		{
			$this->displayer_chrome->set_head_items($this->manager->head_items);
		}
	}
	
	/**
	 * Show the media file content manager as usual.
	 * @return true
	 */
	public function show_form()
	{
		$this->manager->_parent_show_form();
	}
	...
	{
		$this->manager->change_element_type('caption_url', 'hidden');
	}
	
	function modify_audio_description_field()
	{
		$this->manager->change_element_type('audio_description_url', 'hidden');
	}
	...
	{
		$this->manager->set_comments ('name', form_comment('For internal use (this will not be used on the public site)'));
		$this->manager->add_required ('media_format');
		...
		$this->manager->set_display_name ('media_duration', 'Length of A/V Item');
		$this->manager->set_display_name ('media_size', 'File Size');
		$this->manager->set_display_name ('media_quality', 'Quality');
		$this->manager->set_comments ('media_duration', form_comment('The duration in time of the audio or video clip. If you are importing a Quicktime file, this will be automatically determined; otherwise, open the file in your media player to find this info.'));
		$this->manager->set_comments ('media_quality', form_comment('Use this field to indicate either the general quality level of the file (e.g. low/medium/high) or the specific quality-related specs of the file (e.g. "128 kbps" or "16 bit/44.1 khz".)'));
		$this->manager->set_comments ('media_size', form_comment('The size, in Kb or Mb, of the A/V file. If you are importing a file, this will be automatically determined.'));
		$this->manager->set_comments ('height', '<span class="smallText"> pixels (Not needed for audio files)</span><div class="smallText formComment">To determine the width and height of a file, open it in a player and find "get movie info," "clip info," or a similar menu option.</div>');
		$this->manager->change_element_type( 'height', 'text', array('size'=>3));
		$this->manager->set_comments ('width', '<span class="smallText"> pixels (Not needed for audio files)</span>');
		$this->manager->change_element_type( 'width', 'text', array('size'=>3));
		$this->manager->change_element_type( 'reason_managed_media', 'hidden');
		
		$this->manager->change_element_type( 'media_format', 'select_no_sort', array('options'=>array('Quicktime'=>'Quicktime (.mov, .m4v, .m4a, .mpeg, .mpg, .mp4, .aiff)','Windows Media'=>'Windows Media (.avi, .wav, .wma, .wmv)','Real'=>'Real (.rm, .ra)','Flash'=>'Flash Multimedia (.swf)','Flash Video'=>'Flash Video (.flv)','MP3'=>'mp3'),'display_name'=>'Format'));

		$this->manager->set_display_name ('av_part_number', 'Part Number');
		$this->manager->set_display_name ('av_part_total', 'Of');
		$this->manager->change_element_type( 'keywords', 'hidden');
		$this->manager->change_element_type( 'media_size_in_bytes', 'hidden');
		$this->manager->change_element_type( 'description', 'text');
		$this->manager->change_element_type( 'av_part_number', 'text', array('size'=>3));
		$this->manager->change_element_type( 'av_part_total', 'text', array('size'=>3));
		$this->manager->add_comments ('av_part_number', '<span class="smallText"> (e.g. 2)</span>');
		$this->manager->add_comments ('av_part_total', '<span class="smallText"> (e.g. 5)</span>');
		$this->manager->set_display_name ('description', 'Part Description');
		$this->manager->add_comments ('description', form_comment('(e.g. "A Tour of Northfield")'));
		$this->manager->set_display_name ('default_media_delivery_method', 'Default Delivery Method');
		$this->manager->add_element( 'parts_hr_1' , 'hr' );
		$this->manager->add_element( 'parts_hr_2' , 'hr' );
		$this->manager->add_element( 'file_info_header', 'comment', array('text'=>'<h4>File info</h4>'));
	}
	...
	function add_file_preview()
	{
		...
		{	
			...
			{
				$this->manager->add_element( 'file_preview' , 'commentWithLabel' , array('text'=>$html_markup.$problem_note) );
			}
		}
	}
	...
	{
		if(!$this->manager->get_value('name') && !empty($this->manager->admin_page->request['__old_id']))
		{
			$avid = turn_into_int($this->manager->admin_page->request['__old_id']);
			$av = new entity($avid);
			if($av->get_value('name'))
			{
				$this->manager->set_value('name',$av->get_value('name'));
			}
		}
	}
	...
	{
		$this->manager->add_element('parts', 'comment', array('text'=>'<strong>Is this file one of several 
sequential parts</strong> that make up a media work -- like "part 3 of 7?"'));
		$this->manager->add_element( 'is_part' , 'select_no_sort' , 
array('options'=>array('yes'=>'Yes','no'=>'No'),'display_name'=>'&nbsp;'), 'no' );
		if(!$this->manager->get_value('av_part_number') && !$this->manager->get_value('av_part_total') && !$this->manager->get_value('description'))
		{
			$this->manager->set_value('is_part','no');
		}
		else
		{
			$this->manager->set_value('is_part','yes');
		}
	}
	...
	function massage_delivery_method_elements()
	{
		...
		{
			if($this->manager->get_value($key) == 'true')
			{
		...
		if($this->manager->get_value('url'))
		{
			if($this->manager->get_value('reason_managed_media'))
			{
				if($this->manager->get_value('media_is_progressively_downloadable') == 'true')
				{
					$http_url = alter_protocol($this->manager->get_value('url'),'rtsp','http');
				...
				if($this->manager->get_value('media_is_streamed') == 'true')
				{
					$rtsp_url = alter_protocol($this->manager->get_value('url'),'http','rtsp');
		...
		if($this->manages_media || $this->manager->get_value('reason_managed_media'))
		{
			$this->manager->add_element( 'delivery_methods', 'checkboxGroup', array('options'=>$this->media_delivery_methods, 'display_name'=>'Delivery Method(s)' ) );
			$this->manager->set_value('delivery_methods',$delivery_methods_vals);
		}
		else
		{
			$this->manager->add_element( 'delivery_methods', 'radio', array('options'=>$this->media_delivery_methods, 'display_name'=>'Delivery Method' ) );
			reset($delivery_methods_vals);
			$this->manager->set_value('delivery_methods',current($delivery_methods_vals));
			$this->manager->change_element_type( 'default_media_delivery_method','hidden');
		}
		$this->manager->add_required('delivery_methods');
		$this->manager->change_element_type( 'media_is_progressively_downloadable', 'hidden');
		$this->manager->change_element_type( 'media_is_streamed', 'hidden');
	}
	...
	function add_file_import_selector()
	{
		if($this->manages_media)
		{
			...
			if($this->manager->get_value('reason_managed_media'))
			{
				$this->manager->add_element('import_file','select',array('options'=>$media_files,'display_name'=>'Replace file'));
			}
			else
			{
				$this->manager->add_element( 'import_file', 'select_js', 
			...
			if(!empty($media_files))
			{
				$this->manager->add_comments ('import_file', form_comment('Media files in your "home/webpub/reason_import_only" folder. To update the list of available files, add or remove files from your "home/webpub/reason_import_only" folder and refresh this page.'));
			}
			else
			{
				$this->manager->manager->add_comments ('import_file', form_comment('To import a media file, create a folder named "reason_import_only" in your home/webpub directory, place the file in it, and then refresh this page.'));
			}
			if(MEDIA_ALLOW_DIRECT_UPLOAD)
			{
				$this->manager->add_element('import_file_as_alternate', 'comment', array('text'=>'If your file is larger than '.format_bytes_as_human_readable($this->get_actual_max_upload_size()).', you can import the file from your home/webpub/reason_import_only folder. <p style="color: red">NOTE - this import folder (home/webpub/reason_import_only) is a new location - Reason no longer looks for media in the streaming_media folder.</p>'));
			}
		}
	}
	...
	function modify_url_field()
	{
		if($this->manages_media)
		{
			if($this->manager->get_value('reason_managed_media'))
			{
				$this->manager->change_element_type('url','hidden');
			}
			if($this->manager->get_value('url'))
			{
				if($this->manager->_is_element( 'upload_file' ))
					$this->manager->set_display_name ('upload_file', 'Upload replacement file');
				if($this->manager->_is_element( 'import_file' ))
					$this->manager->set_display_name ('import_file', 'Import replacement file');
				$this->manager->add_element('replacement_header','comment',array('text'=>'<h4>Replace file</h4>'));
			}
			else
			{
				$this->manager->set_display_name('url','or provide the address of a file already on the web');
				$this->manager->add_element('replacement_header','comment',array('text'=>'<h4>File location</h4>'));
			}
		}
	}
	...
	*/
	...
	{		
		if($this->manager->get_value('reason_managed_media') && $this->manages_media)
		{
			$user = new entity( $this->manager->admin_page->user_id );
			$stream = new $this->file_transfer_utility_class( $user->get_value('name') );
			$stream->set_meta_info($this->manager->get_value('id'),$this->get_meta_info());
		}
	}
	...
	*/
	...
	{
		if(!$this->manager->has_error('delivery_methods'))
		{
			$delivery_methods_chosen = $this->manager->get_value('delivery_methods');
			foreach($this->media_delivery_methods as $field=>$desc)
			{
				...
				{
					$this->manager->set_value($field, 'true');
				}
				else
				{
					$this->manager->set_value($field,'false');
				}
			}
		}
	}	
	...
	{
		if($this->manager->get_value('media_is_progressively_downloadable') == 'true' && $this->manager->get_value('media_is_streamed') == 'true')
		{
			if(!$this->manager->get_value('default_media_delivery_method'))
			{
				$this->manager->set_value('default_media_delivery_method','progressive_download');
			}
		}
		elseif($this->manager->get_value('media_is_progressively_downloadable') == 'true')
		{
			$this->manager->set_value('default_media_delivery_method','progressive_download');
		}
		elseif($this->manager->get_value('media_is_streamed') == 'true')
		{
			$this->manager->set_value('default_media_delivery_method','streaming');
		}
	}	
	...
	{
		if($this->manager->get_value('is_part') == 'yes')
		{
			if(!$this->manager->get_value('description') && !$this->manager->get_value('av_part_number') && !$this->manager->get_value('av_part_total'))
			{
				$this->manager->set_error('is_part','If this is a one of several sequential files that make up a media work, please enter the part number, total, and/or description');
			}
			elseif(!$this->manager->get_value('av_part_number') && $this->manager->get_value('av_part_total'))
			{
				$this->manager->set_error('av_part_number','Please enter which part this file is.');
			}
		}
		elseif($this->manager->get_value('is_part') == 'no')
		{
			$this->manager->set_value('av_part_number',0);
			$this->manager->set_value('av_part_total',0);
			$this->manager->set_value('description','');
		}
	}
	...
	{
		return $stream->get_source_md5sum( $this->manager->get_value('import_file') );
	}	
	
	function transfer_uploaded_file($stream,$user,$local_temp_location)
	{
		if(!$stream->place_local_media($this->manager->get_value('id')))
		{
			$this->manager->set_error('upload_file','File transfer wasn\'t successful. Reason: '.$stream->get_last_error() );
	...
	{
		if($this->manager->get_value('default_media_delivery_method') == 'streaming')
		{
			return 'rtsp';
		}
		else
	...
	{
		if(!$stream->place_media($this->manager->get_value('id'), $this->manager->get_value('import_file')))
		{
			$this->manager->set_error('import_file','File transfer wasn\'t successful. Reason: '.$stream->get_last_error() );
			trigger_error('File transfer wasn\'t successful. Reason: '.$stream->get_last_error());
		}
		else
		{
	...
	{
		$parsed = parse_url($this->manager->get_value('url'));
		$protocol = $this->get_protocol();
		if($parsed['scheme'] != $protocol)
		{
			$new_url = alter_protocol($this->manager->get_value('url'),$parsed['scheme'],$protocol);
			$this->manager->set_value('url', $new_url);
		}
	}
	...
	function check_url_sanity()
	{
		if(!$this->manager->get_value('reason_managed_media') && $this->manager->get_value('url'))
		{
			...
			if($this->manager->get_value('media_is_progressively_downloadable') == 'true' && array_key_exists($state,$progressive_error_conditions))
			{
				$this->manager->set_error('delivery_methods','<strong>URL does not support progressive downloading.</strong><br /> '.$progressive_error_conditions[$state]);
			}
		}
	}
}
