 ...
 * @subpackage scripts
 */
$GLOBALS['_reason_upgraders']['4.6_to_4.7']['setup_profiles'] = 'ReasonUpgrader_47_SetupProfiles';
...
class ReasonUpgrader_47_SetupProfiles implements reasonUpgraderInterface
{
	protected $user_id;
	...
	var $relationships = array(
		...
		'profile_to_feature_image' => array (
			'left' => 'profile_type',
			'right' => 'image',
			'details' => array(
				'description'=>'Profile to Feature Image',
				'directionality'=>'unidirectional',
				'connections'=>'many_to_many',
				'required'=>'no',
				'is_sortable'=>'yes',
	...
	{
		if($this->profiles_type_exists() && $this->profiles_relationships_exist())
		{
		...
		{
			$str = '';
			if (!$this->profiles_type_exists()) $str .= '<p>Would create profiles type</p>';
			if (!$this->profiles_relationships_exist()) $str .= '<p>Would create some or all profiles relationships.</p>';
			return $str;
		}
	}
	...
	public function run()
	{
		...
		else
		{
			...
			if (!$this->profiles_relationships_exist())
			{
	...
	protected function profiles_relationships_exist()
	{
