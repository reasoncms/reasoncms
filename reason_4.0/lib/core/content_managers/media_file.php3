 ...
 */
	...
	if(!defined('REASON_AV_TRANSFER_UTILITY_LOCATION'))
	{
		define('REASON_AV_TRANSFER_UTILITY_LOCATION','');
	}
	if(!defined('REASON_AV_TRANSFER_UTILITY_CLASS_NAME'))
	{
		define('REASON_AV_TRANSFER_UTILITY_CLASS_NAME','');
	}
	if(!defined('REASON_MANAGES_MEDIA'))
	{
		define('REASON_MANAGES_MEDIA',false);
	}
	if(!defined('NOTIFY_WHEN_MEDIA_IS_IMPORTED'))
	{
		define('NOTIFY_WHEN_MEDIA_IS_IMPORTED',false);
	}
	if(!defined('MEDIA_FILESIZE_NOTIFICATION_THRESHOLD'))
	{
		define('MEDIA_FILESIZE_NOTIFICATION_THRESHOLD',0);
	}
	if(!defined('MEDIA_ALLOW_DIRECT_UPLOAD'))
	{
		define('MEDIA_ALLOW_DIRECT_UPLOAD',true);
	}
	
	if(!defined('MEDIA_ALLOW_IMPORT_FROM_FILESYSTEM'))
	{
		define('MEDIA_ALLOW_IMPORT_FROM_FILESYSTEM',true);
	}
	if(!defined('MEDIA_MAX_UPLOAD_FILESIZE_MEGS'))
	{
 ...
 */
	...
	class avFileManager extends ContentManager
	{
		...
		/**
		 * The content manager uses logic to ficure out if it has all the information
		...
		var $actual_max_upload_size_bytes;
		...
		function alter_data()
		{
			$this->actual_max_upload_size_bytes = $this->get_actual_max_upload_size();
...
array('options'=>array('yes'=>'Yes','no'=>'No'),'display_name'=>'&nbsp;'), 'no' );
		...
		function add_file_import_selector()
		{
			if($this->manages_media)
			{
				...
				if(MEDIA_ALLOW_DIRECT_UPLOAD)
				{
					$this->add_element('import_file_as_alternate', 'comment', array('text'=>'If your file is larger than '.format_bytes_as_human_readable($this->actual_max_upload_size_bytes).', you can import the file from your home folder.'));
				}
			}
		}
		...
		function check_url_sanity()
		{
			if(!$this->get_value('reason_managed_media') && $this->get_value('url'))
			{
				...
				if($this->get_value('media_is_progressively_downloadable') != 'true')
				{
					$streaming_xtra_text = ' and check " Progressive Download"';
				} */
				...
				/* $streaming_error_conditions = array(
									'not_streaming'=>'Web address that start with "http" and <em>neither</em> have the extension ".ram" nor use a folder called "ramgen" can only be delivered using the "Progressive Download" method. <strong>Uncheck "Streaming"'.$streaming_xtra_text.' to continue.</strong>'); */
			...
			/*	if($this->get_value('media_is_streamed') == 'true' && array_key_exists($state,$streaming_error_conditions))
				{
					$this->set_error('delivery_methods','<strong>URL does not support streaming.</strong><br /> '.$streaming_error_conditions[$state]);
				} */
			}
		}
		...
		function run_error_checks()
		{
			...
			if($this->_is_element( 'upload_file' ))
			{
				...
				if( ($file->state == 'uploaded' OR $file->state == 'pending') AND file_exists( $file->tmp_full_path ) )
				{
					$file_uploaded = true;
					$user = new entity( $this->admin_page->user_id );
					$stream = new $this->file_transfer_utility_class( $user->get_value('name'), $file->tmp_full_path );
