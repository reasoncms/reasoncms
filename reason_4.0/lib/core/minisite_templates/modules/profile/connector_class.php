...
{	
	...
	/**
	  * Given the slug of a tag section, this method returns the corresponding
	  * Reason relationship.
	  *
	  * @param string Profile section
	  * @return string $rel Relationship name
	  *
	  */
	function get_relationship_from_section_id($slug)
	{
		if (isset($this->config->tag_section_relationship_names[$slug])) return $this->config->tag_section_relationship_names[$slug];
		return false;
	}
		...
		foreach ($this->config->tag_section_relationship_names as $section => $rel)
		{
			...
			if ( ($profile_id = $p->get_profile_id()) && ($rel_id = relationship_id_of($rel)))
			{
				...
				if ($results = $es->run_one())
				{
					...
					{
						if ($tag = $this->get_tag_by_id((int)$result->get_value('id')))
						{
			...
			foreach ($connections as $section => $tags)
			{
				foreach ($tags as $tag => $tag_data)
				{
					foreach ($tag_data['profiles'] as $rel => $profiles)
					{
						...
						{	
							// See if there's an affiliation in the directory record and if
							// it matches the allowed affiations.						
							$affil = (isset($dir_data[$profile]['ds_affiliation'])) ? reset($dir_data[$profile]['ds_affiliation']) : 'none';
							if (isset($dir_data[$profile]) && isset($this->affiliations[$affil]))
							{
	...
	/**
	  ...
	  *
	  * @param string $slug
	  * @param boolean $include_children Whether to include connections for child tags of slug
	  * @return array
	  */
	function get_connections_for_tag($slug, $include_children=true)
	{
		$connections = $profile_ids = array();
		...
		if ($include_children && isset($tag['children']))
		{
	...
	/** Generates an array of connection data for a given tag. The returned array looks like this:
	  * array(section_name => array(tag_id => tag_data))
	  *
	  * The data is "raw" because the profile data isn't expanded -- all you get are ids.
	  *
	  * @param string $slug
	  * @param boolean $include_children Whether to include connections for child tags of slug
	  * @return array
	  */
	function get_raw_connections_for_tag($slug, $include_children=true)
	{
		$connections = $profile_ids = array();
		if (!($tag = $this->get_tag_by_slug($slug))) return $connections;
		...
		if ($results = $es->run_one())
		{
			foreach($results as $result)
			{
				if ($children = $result->get_value('child_id'))
				{
					...
					{
						$this->tag_cache[(int)$result->id()]['children'][(int)$id] = (int)$id;
						$this->tag_cache[(int)$id]['parents'][(int)$result->id()] = (int)$result->id();
					}
				}
			}
		}
	...
	{
		if (isset($this->tag_cache[(int)$id]))
			return $this->tag_cache[(int)$id];
		else
		{
			if (!$this->refreshed)
			{
	...
	/**
	  * Given a tag name, return a populated tag array. Requires the tag cache to be loaded.
	  *
	  * @param string $name
	  */
	...
	public function get_top_tags_for_relation($rel)
	{
		...
		if (isset($this->relation_index[$rel]))
		{
			...
			{
				if (isset($tag['profiles'][$rel]))
					$top[(int)$tag['id']] = count($tag['profiles'][$rel]);
			}
		}
		arsort($top);
		return $top;
	...
	{
		if (empty($this->tag_cache[(int)$id]['children'])) return array();
		foreach($this->tag_cache[(int)$id]['children'] as $child_id) 
			$children[(int)$child_id] = $this->tag_cache[(int)$child_id];
	...
	{
		if (isset($this->tag_cache[(int)$parent_id]) && isset($this->tag_cache[(int)$child_id]))
		{
			$this->tag_cache[(int)$parent_id][$prefix.'children'][(int)$child_id] = (int)$child_id;
			$this->tag_cache[(int)$child_id][$prefix.'parents'][(int)$parent_id] = (int)$parent_id;
	...
	public function clear_cache_relationship($parent_id, $child_id, $save = false, $prefix = '')
	{
		if (isset($this->tag_cache[(int)$parent_id]) && isset($this->tag_cache[(int)$child_id]))
		{
			unset($this->tag_cache[(int)$parent_id][$prefix.'children'][(int)$child_id]);
			unset($this->tag_cache[(int)$child_id][$prefix.'parents'][(int)$parent_id]);
	...
	public function update_tag_cache($changes, $save = false)
	{
		...
		{
			if ($values == null)
				unset($this->tag_cache[(int)$id]);
			else if (empty($this->tag_cache[(int)$id]))
				$this->tag_cache[(int)$id] = $values;
			else
				$this->tag_cache[(int)$id] = array_merge($this->tag_cache[(int)$id], $values);
		}
	...
	public function get_slug_index($rebuild=false)
	{
		...
		{
			if (!empty($tag['slug']))
				$this->slug_index[$tag['slug']] = (int) $id;
			else
		...
		foreach ($this->config->tag_section_relationship_names as $relationship)
		{
			...
			if ($results = $es->run_one())
			{
				foreach($results as $result)
				{
					...
					if (isset($this->tag_cache[(int)$result->id()]['count']))
						$this->tag_cache[(int)$result->id()]['count'] += $profile_count;
					else
					{
						$this->tag_cache[(int)$result->id()]['id'] = (int)$result->id();
						$this->tag_cache[(int)$result->id()]['name'] = $result->get_value('name');
						$this->tag_cache[(int)$result->id()]['slug'] = $result->get_value('slug');
						$this->tag_cache[(int)$result->id()]['count'] = $profile_count;
						$this->tag_cache[(int)$result->id()]['created'] = $result->get_value('creation_date');
					}
					if ($profiles)
					{
						foreach ($profiles as $profile)
							$this->tag_cache[(int)$result->id()]['profiles'][$relationship][$profile] = $profile;
					}
				}
			}	
		}
