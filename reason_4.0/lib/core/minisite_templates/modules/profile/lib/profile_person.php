...
class profilePerson
{
	...
	private function create_profile_entity_if_needed()
	{
		...
		if (is_null($profile))
		{
			...
			reason_create_entity( id_of($this->config->site_unique_name), id_of('profile_type'), get_user_id('causal_agent'), $this->construct_profile_name(), array('new' => 0, 'user_guid' => $this->get_first_ds_value('ds_guid')) );
	...
	/**
	 * Get the affiliation display name.
	 *
	 * @param boolean $show_class_year Whether to include the class year in the display name
	 */
	public function get_affiliation_display_name()
	{
		$name = $this->get_first_ds_value('ds_affiliation');
		return (!empty($name) && isset($this->config->affiliations_that_have_profiles[$name])) ?
				$this->config->affiliations_that_have_profiles[$name] : 
				'';
	}
	...
	public function set_category($slug, $name, $section)
	{
		if ($slug && $name && ($rel_name = $this->get_tag_section_relationship_name($section)) && ($rel_id = relationship_id_of($rel_name)))
		{
			...
			{
				$category_id = reason_create_entity( id_of($this->config->site_unique_name), id_of('category_type'), get_user_id('causal_agent'), $name, array('new' => 0, 'slug' => $slug) );	
			}
	 ...
	 *
	 * @param sites
	 * @param scope_array if set we only pay attention to site names in the scope
	 * @param delete_unused default true
	 *
	 * @param array sites array with name => url pairs.
	 * @return array synced_sites - output from get_sites()
	 */
	public function sync_sites($sites, $scope_array=array(), $delete_unused=true)
	{
		$made_changes = false;
		if ($site_entities = $this->get_sites_entities())
		{
			foreach ($site_entities as $id=>$site)
			{
				...
				if (empty($scope_array) || in_array($name, $scope_array))
				{
					if (isset($sites[$name]))
					{
						if ($url != $sites[$name])
		...
		if (!empty($sites))
		{
			...
			foreach ($sites as $name => $url) // create entities and rels.
			{
				if (empty($scope_array) || in_array($name, $scope_array))
				{
					...
					$eid = reason_create_entity( id_of($this->config->site_unique_name), id_of('external_url'), $uid, $name_with_username, array('new' => 0, 'url' => $url) );
	...
	public function set_image($image)
	{
		...
		if( !empty($image->tmp_full_path) AND file_exists( $image->tmp_full_path ) )
		{
			...
			if ($id = reason_create_entity( id_of($this->config->site_unique_name), id_of('image'),$owner, $this->construct_profile_name().' Profile Image', $values))
			{
	...
	{
		$link = false;
		if ($image_entity = $this->get_image_entity( $refresh ))
			$link = reason_get_image_url($image_entity->id());
		
		return $link;
	}
	
	public function get_image_entity( $refresh = false )
	{
		if(!isset($this->image)|| $refresh)
		{
			$this->image = false;
			if ($profile = $this->get_profile_entity())
			{
	...
	public function set_resume( $file )
	{
		...
		{
			$file['name'] = $this->get_standardized_resume_filename($file['name']);
			$asset_id = reason_create_asset(id_of($this->config->site_unique_name), $this->get_updater_user_id(), $file);
