...
{	
	...
	{
		if (empty($this->pc)) $this->pc = new $this->config->connector_class();
		$person = $this->get_person();
		$sections = $this->pc->get_connections_for_user($person);
		...
		foreach ($sections as $section => $tags)
			$str .= '<li id="tab'.$section.'"><a href="#'.$section.'">'.$this->pc->get_section_name($section, $person).'</a></li>';
		$str .= '</ul>';
		$str .= '<div id="tagInfo">';		
		foreach ($sections as $section => $tags)
		{
			$str .= '<div class="connectSection" id="section'.$section.'">';
			$str .= '<h3 class="sectionName"><a name="'.$section.'"></a>'.$this->pc->get_section_name($section, $person).'</h3>';
			$str .= '<ul class="tag">';
			...
			foreach ($tags as $id => $tag_data)
			{
				...
				foreach ($profiles_by_affil as $affil => $profiles)
				{
					...
					{
						$classes = ( $count < $this->max_connections_shown ) ? array() : array('overflow');
						$str .= $this->get_profile_list_item($profile, $classes);
						$count++;
					}
					$str .= '</ul>';
				}
	...
	/**
	 * Get the HTML for a single profile connections item, optionally passing classes.
	 */
	protected function get_profile_list_item($profile, $classes=array())
	{
		if ($classes)
			$str = '<li class="'.join(' ', $classes).'">';
		else
			$str = '<li>';	
		$str .= '<a href="'. $this->site_url . $profile['ds_username'][0] .'">'.$profile['display_name'].'</a></li>'."\n";
		return $str;
	}
	...
	protected function get_site_categories($edit_section='tags', $filter=null)
	{
		...
		if ( $cache_tags = $this->pc->get_tag_index_by_relation($rel) )
		{
