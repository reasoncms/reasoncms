 ...
 */
	...
	class GalleryModule extends DefaultMinisiteModule
	{
		...
		function init( $args = array() ) // {{{
		{
			...
			{
				$es->add_relation( '(entity.name LIKE "%'.addslashes($this->request[ 'search_image' ]).
								   '%" OR meta.description LIKE "%' . addslashes($this->request[ 'search_image' ]) . '%"'.
								   ' OR meta.keywords LIKE "%' . addslashes($this->request[ 'search_image' ]) . '%"'.
								   ' OR chunk.content LIKE "%' . addslashes($this->request[ 'search_image' ]) . '%"'.
								   ')' );
			}
