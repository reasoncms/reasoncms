...
class EventSignupModule extends EventRegistrationModule
{
	function process_registration()
	{
		...
		update_entity( $this->event->id(), $this->event->get_value('last_edited_by'), values_to_tables( get_entity_tables_by_id( $this->event->id() ), array('registration'=>'full','show_hide'=>'hide', 'content'=>$this->event->get_value('content').'<h3>Registration Information</h3>'.nl2br($body) ) ), true );
