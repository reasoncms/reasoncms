...
class reasonFeedDisplay
{
	...
	function display_feed ( $mode="feed" )
	{
    	...
    	{
					$rssNum = count($rss->items);
					if($this->options['num_posts'] != "0" && $this->options['num_posts'] < $rssNum){
						$rssNum = intval($this->options['num_posts']);
					}
  	    	for ($x=0;$x<$rssNum;$x+=1) {
            $item = $rss->items[$x];
			...
			$filterFields = ['title'=>'title', 'description'=>'description', 'Any'=>'Any'];
			...
			$filter_match = true;
			if($this->options['field_title'] != "" && $this->options['field_words']!=""){
				$filter_match = false;
				$fieldTitle = strtolower($this->options['field_title']);
				$fieldWordsStr = $this->options['field_words'];
		    ...
		    // skip events in the future if 'show future posts' set anything but yes
			...
			if((($date - $now) < 0 || $this->options['future_posts'] == "Yes") && $search_match && $title != '' && ( $description != '' || $this->_show_entries_lacking_description == true) && $href != '' && $filter_match){
	...
	function get_parsed_data()
	{
		...
		{
			$rss = fetch_rss(htmlspecialchars_decode($this->_feed_location));
		}
		else
		{
