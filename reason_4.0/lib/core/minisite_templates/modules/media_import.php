...
$GLOBALS[ '_module_class_names' ][ basename( __FILE__, '.php' ) ] = 'mediaUploadModule';

/**
 * A minisite module that allows a user to upload a video or audio file.
 */
class mediaUploadModule extends DefaultMinisiteModule
{
	...
	var $acceptable_params = array(
		...
		'default_group_uname' => '',
		'integration_library' => '',
		'show_hide_default' => 'show',
		'require_login' => false,
	);
	
	var $cleanup_rules = array(
		'thanks' => 'turn_into_int', 
		// TODO: these cleanup rules?
	);
	...
	protected $_filename;
	protected $_user_id;
	protected $_integration_library;
	protected $modifier;
	protected $shim;
	...
	protected function _get_form()
	{
		...
		$av_types = $this->shim->get_supported_av_types();
		if (count($av_types) == 1)
		{
			$d->add_element('av_type', 'hidden');
			$d->set_value('av_type', current($av_types));
		}
		else
		{
			$audio_blurb = 'Audio <span class="smallText formComment">(e.g. sound-only music, speech, etc.)</span>';
			$video_blurb = 'Video <span class="smallText formComment">(e.g. movies, videos, etc.)</span>';
			$d->add_element('av_type', 'radio_no_sort', array('options'=>array('Audio'=>$audio_blurb, 'Video'=>$video_blurb)));
		}
		...
		$params = array(//'authenticator' => $authenticator,
			'acceptable_extensions' => $this->shim->get_recognized_extensions(),
		...
		$d->add_comments('upload_file',form_comment('File must have one of the following extensions: .'.implode(', .', $this->shim->get_recognized_extensions()) ) );
		$d->add_comments('upload_file',form_comment('<div class="maxUploadSizeNotice">Maximum file size for uploading is '.MEDIA_MAX_UPLOAD_FILESIZE_MEGS.'MB. </div>'));
		
		$d->add_element('url', 'text');
		$d->set_display_name('url', 'Media url');
		$d->add_comments('url', form_comment('If you are uploading a file larger than '.MEDIA_MAX_UPLOAD_FILESIZE_MEGS.'MB, enter the url of the file you are uploading here.'));
		$d->add_comments('url', form_comment('Example: http://people.carleton.edu/~[USERNAME]/reason_import_only/[FILENAME]'));
	...
	public function error_check_form($disco)
	{
		...
		elseif ($disco->get_value('url'))
		{
			// check to see if url exists
			$name = basename($disco->get_value('url'));
			$encoded_url = str_replace($name, rawurlencode($name), $disco->get_value('url'));
			...
			if ($retcode != 200)
			{
				$disco->set_error('upload_file', 'The provided url was invalid.');
				return;
			}
		}
	...
	{
		if ($this->_media_work_id) {
			$this->_email_confirmation($disco, $this->_media_work_id);
			$this->_send_contact_email($disco, $this->_media_work_id);
		}
	}
	...
	function get_media_import_dir()
	{
		return WEB_PATH.WEB_TEMP;
	}
