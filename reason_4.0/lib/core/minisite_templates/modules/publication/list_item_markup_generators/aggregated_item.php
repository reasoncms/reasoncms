...
class AggregatorListItemMarkupGenerator extends PublicationMarkupGenerator
{
	...
	function run ()
	{
		...
		$this->markup_string .= $this->get_publication_markup();
...
/////
	...
	function get_publication_markup()
	{
		$pub = $this->passed_vars['item_publication'];
		if(!empty($pub))
		{
			return '<div class="pubName">'.$pub->get_value('name').'</div>';
		}
		return '';
	}
