<?php
	...
	reason_include_once( 'minisite_templates/foundation.php' );
	reason_include_once( 'classes/module_sets.php' );
	
	// this variable must be the same as the class name
	$GLOBALS[ '_minisite_template_class_names' ][ basename( __FILE__) ] = 'FoundationLoginTemplate';
	
	class FoundationLoginTemplate extends FoundationTemplate
	{

		function get_body_tag_classes()
		{
			// Adds the class "foundationLogin" to the body tag.
			$classes = array();
			$classes[] = 'body foundationLogin';
		...
		function foundation_you_are_here($delimiter = '')
		{
		}
