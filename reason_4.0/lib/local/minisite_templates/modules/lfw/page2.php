...
//
//    Work on the second page of the Lutheran Festival of Writing registration form
//
////////////////////////////////////////////////////////////////////////////////
...
class LFWFormPageTwo extends FormStep
{
	...
	var $error;
	var $expense_budget_number = '11-102-30600-51111';
	var $revenue_budget_number = '11-000-30600-44906';
	var $transaction_comment = 'LFW Reg';
	...
	var $elements = array(
		'review_note' => array(
			'type' => 'comment',
			'text' => 'Lutheran Festival of Writing registration overview',
		),
		'payment_note' => array(
			'type' => 'comment',
...
//			'size'=>35,
	...
	var $display_name = 'Registration Review / Card Info';
	...
	function post_error_check_actions()
	{
		if ($this->show_form)
		{
			...
			$text .= '<p class="changeRegistrationButton"><a href="?_step=LFWFormPageOne">Change Registration Information</a></p>'."\n";
	...
	function get_confirmation_text()
	{
		...
		if ($this->controller->get('middle_initial')){ 
			$txt .= $this->controller->get('middle_initial').' ';
		}
		$txt .= $this->controller->get('last_name').'</li>'."\n";
...
//		}
		...
		if ($this->controller->get('office_phone'))
		{
			$txt .= '<li><strong>Office Phone:</strong> '.$this->controller->get('office_phone').'</li>'."\n";
		}
		...
		if ($this->controller->get('institution'))
		{
			$txt .= '<li><strong>Institution:</strong> '.$this->controller->get('institution').'</li>'."\n";
		}
		if($this->controller->get('position_title'))
		{
			$txt .= '<li><strong>Position Title:</strong> '.$this->controller->get('position_title').'</li>'."\n";
		}		
		if($this->controller->get('profession') )
		{
			$txt .= '<li><strong>Profession:</strong> '.($this->controller->get('profession')).'</li>'."\n";
		}
		if($this->controller->get('conference_fee'))
		{
			$txt .= '<li><strong>Conference Fee:</strong> $'.number_format( $this->controller->get('conference_fee'), 2, '.', ',' ).'</li>'."\n";
		}
		...
		{
			$txt .= '<li><strong>Special dietary needs:</strong> '.$this->controller->get('dietary_needs').'</li>'."\n";
		}
		...
		if( !$this->_has_errors() )
		{
			...
			else
			{
				...
				if (reason_unique_name_exists('lfw_thank_you_blurb'))
					$confirm_text_with_blurb = get_text_blurb_content('lfw_thank_you_blurb') . $confirm_text;
				else
					$confirm_text_with_blurb = '<p><strong>Thank you for registering for the Lutheran Festival of Writing!</strong></p>' . $confirm_text;
