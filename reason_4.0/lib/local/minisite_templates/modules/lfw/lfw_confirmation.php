...
* 	based off GiftConfirmation Matt Ryan, 29 March 2005
* 
* This class wraps up the logic needed to get the confirmation 
* text out of the homecoming_registration database in a (fairly) secure way.  It relies 
...
* Sample code
* <code>
* $foo = new HomecomingConfirmation;
...
class LfwConfirmation
{
	...
	* @var string
	*/
	var $error_message = 'We\'re sorry.  Your Lutheran Festival of Writing registration was successfully processed, but there appears to have been an error generating your confirmation information.  Please contact the Alumni Office for assistance.';
	...
	*/
	function validates()
	{
		if(!$this->_validation_has_run)
		{
			...
			{
				$lfwPF = new lfwPF;
				$confirm_text = $lfwPF->get_confirmation_text($this->reference_number);
				if(!empty($confirm_text))
				{
					if($this->hash == $this->make_hash($confirm_text))
					{
						if (reason_unique_name_exists('lfw_thank_you_blurb'))
							$confirm_text = get_text_blurb_content('lfw_thank_you_blurb') . $confirm_text;
						else
							$confirm_text = '<p><strong>Thank you for registering for the Lutheran Festival of Writing!</strong></p>' . $confirm_text;
