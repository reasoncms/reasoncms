<?php
	...
	$GLOBALS[ '_module_class_names' ][ basename( __FILE__, '.php' ) ] = 'CalloutBlurbModule';
	
	class CalloutBlurbModule extends BlurbModule
	{
		...
		function run() // {{{
		{
			...
			foreach( $this->blurbs as $blurb )
			{
				...
				if (preg_match("/callout_blurb/", $blurb->get_value('unique_name')))
				{
