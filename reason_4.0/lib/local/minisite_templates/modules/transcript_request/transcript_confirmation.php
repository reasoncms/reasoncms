...
* text out of the transcript_requests database in a (fairly) secure way.  It relies 
...
* Sample code
* <code>
* $foo = new TranscriptConfirmation;
...
class TranscriptConfirmation
{
	...
	* @var string
	*/
	var $error_message = 'We\'re sorry.  Your transcript request was successfully processed, but there appears to have been an error generating your confirmation information.  Please contact the Office of the Registrar for assistance.';
				...
				if(!empty($confirm_text))
				{
					if($this->hash == $this->make_hash($confirm_text))
					{
						if (reason_unique_name_exists('transcript_thank_you_blurb'))
							$confirm_text = get_text_blurb_content('transcript_thank_you_blurb') . $confirm_text;
						else
