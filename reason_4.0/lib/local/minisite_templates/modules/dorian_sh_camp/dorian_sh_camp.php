...
$GLOBALS[ '_module_class_names' ][ 'dorian_sh_camp/'.basename( __FILE__, '.php' ) ] = 'DorianSHCampModule';

class DorianSHCampModule extends DefaultMinisiteModule
{
    /**
    ...
    function pre_request_cleanup_init()
    {
        ...
        reason_include_once( 'minisite_templates/modules/dorian_sh_camp/page1.php' );
        reason_include_once( 'minisite_templates/modules/dorian_sh_camp/page2.php' );
        reason_include_once( 'minisite_templates/modules/dorian_sh_camp/page3.php' );

        $this->controller = new FormController;
        $this->controller->set_session_class('Session_PHP');
        $this->controller->set_session_name('REASON_SESSION');
        $this->controller->set_data_context('dorian_sh_camp');
    ...
    function run()
    {
        if( !empty( $this->request[ 'r' ] ) AND !empty( $this->request[ 'h' ] ) )
        {
            ...
            $tc = new DorianShCampConfirmation;
            ...
            // MUST reconnect to Reason database.
                        // DorianSHCampConfirmation connects to dorian_sh_camp for info.
            connectDB( REASON_DB );
        }
        else
        {
