...
class CreditCardThorForm extends DefaultThorForm
{
	...
	function on_every_time()
	{
		...
		$this->change_element_type($this->get_element_name_from_label('payment_amount'),'select',array(
			'display_name'=>'tickets', 'add_null_value_to_top' => true, 'options'=>array(
				'$'.number_format(1*$ticket_cost[1],2,'.','')=>'1 ticket, $' . number_format(1*$ticket_cost[1],2,'.',''),
				'$'.number_format(2*$ticket_cost[1],2,'.','')=>'2 tickets, $' . number_format(2*$ticket_cost[1],2,'.',''),
				'$'.number_format(3*$ticket_cost[1],2,'.','')=>'3 tickets, $' . number_format(3*$ticket_cost[1],2,'.',''),
				'$'.number_format(4*$ticket_cost[1],2,'.','')=>'4 tickets, $' . number_format(4*$ticket_cost[1],2,'.',''),
				'$'.number_format(5*$ticket_cost[1],2,'.','')=>'5 tickets, $' . number_format(5*$ticket_cost[1],2,'.',''),
				'$'.number_format(6*$ticket_cost[1],2,'.','')=>'6 tickets, $' . number_format(6*$ticket_cost[1],2,'.',''),
				),
			)
		);
