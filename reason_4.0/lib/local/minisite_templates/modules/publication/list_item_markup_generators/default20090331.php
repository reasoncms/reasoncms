...
/////
	...
	function get_description_markup()
        /* Luther modified: Brian Jones, April 2009 */
	{
		$item = $this->passed_vars['item'];
		if($item->get_value('description'))
                $markup_string = '<div class="desc">'.$item->get_value('description').'</div>'."\n";
                //$markup_string = preg_replace("|\<\/p\>\s?\<\/div\>|", "  Read more...", $markup_string);
if(!empty($this->passed_vars['link_to_full_item']))
                       $markup_string .= $this->get_more_link_markup();
                //$markup_string = preg_replace("|\<\/p\>\s?\<\/div\>|", "  Read more...", $markup_string);
                //$markup_string = preg_replace("|\<\/p\>\s?\<\/div\>\s?\<\/li\>\s?\<li class\=\"more\"\>\s?\<a href\=|", "  Read more...", $markup_string);
                $markup_string = preg_replace("|\<\/p\>\s?\<\/div\>|", "  Read more...</p></div>", $markup_string);
