...
//
// @todo I don't think this is being used anymore. I quick code check says no. And I don't think it works properly, either.
{
    if ($_SERVER['REMOTE_ADDR'] == '127.0.0.1' // localhost
        ...
        // West Union: Go to http://www.my-ip-address-is.com/city/Iowa/WestUnion-IP-Addresses
        ...
        // Spring Grove: Go to http://www.my-ip-address-is.com/city/Minnesota/SpringGrove-IP-Addresses
...
function luther_is_luther_ip(){
    if (preg_match("/^(192\.203\.196\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]))$/", $_SERVER['REMOTE_ADDR'])
        || preg_match("/^(198\.133\.77\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]))$/", $_SERVER['REMOTE_ADDR'])
        || preg_match("/^(209\.56\.59\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]))$/", $_SERVER['REMOTE_ADDR'])
        || preg_match("/^(74\.207\.(3[2-9]|4[0-9]|5[0-9]|6[0-3])\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]))$/", $_SERVER['REMOTE_ADDR']))
    {
