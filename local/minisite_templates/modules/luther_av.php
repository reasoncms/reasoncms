			...
			//	if (preg_match("/(^http:\/\/www\.youtube\.com\/)(\w+)(\/(.*?)$)/", $vurl, $m ))
				if (preg_match("/(^http:\/\/www\.youtube\.com\/)(watch\?v\=)((.*?)$)/", current($avfilelist)->get_value('url'), $m ))
				{
					if (!$this->params['full_size'])
					{
						...
						echo "<a href=\"" . $m[1] . "embed/" . $m[3] . "?autoplay=1\" onclick=\"return hs.htmlExpand(this, { slideshowGroup: -1, objectType: 'iframe', width: " . current($avfilelist)->get_value('width') . ", height: " . current($avfilelist)->get_value('height') . ", objectLoadTime: 'after', outlineType: 'rounded-white', wrapperClassName: 'draggable-header no-footer'} )\" class=\"highslide\" name=\"" . $vn . "\"><img src=\"http://img.youtube.com/vi/" . $m[3] . "/default.jpg\" /><img class=\"av-play\" title=\"Play Video: " . preg_replace('|\"|', '&quot;', $item->get_value( 'name' )) . "\" src=\"/images/play_44.png\" /></a>";
					}
					else
					{
						...
						echo "<iframe width=\"444\" height=\"356\" src=\"" .$m[1] . "embed/" . $m[3] ."\" frameborder=\"0\" allowfullscreen</iframe>"."\n";
					}
